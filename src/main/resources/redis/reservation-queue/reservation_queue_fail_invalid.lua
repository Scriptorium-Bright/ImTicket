...
if status == 'FAILED_FINAL' then
    if stored_stream_id == ARGV[2]
            and redis.call('HGET', KEYS[8], 'failureSchemaVersion') == ARGV[6]
            and redis.call('HGET', KEYS[8], 'errorCode') == ARGV[7] then
...
redis.call('ZADD', KEYS[7], 'GT', tonumber(ARGV[4]) + tonumber(ARGV[5]),
