...
def main():
    ...
    out += [
        ...
        "",
        f"- layout root end: {fmt(current_layout)} ms -> {fmt(candidate_layout)} ms ({pct(current_layout, candidate_layout):.1f}% change)",
        f"- availability root end: {fmt(current_availability)} ms -> {fmt(candidate_availability)} ms ({pct(current_availability, candidate_availability):.1f}% change)",
        "",
        "## 해석",
        "",
